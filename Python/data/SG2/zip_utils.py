...
class ImageFolderDataset(Dataset):
    ...
    def _open_file(self, fname: str) -> Optional[IO]:
        #print(f"Attempting to open file: {fname}")  # Debug print
        if self._type == 'dir':
            return open(os.path.join(self._path, fname), 'rb')
        if self._type == 'zip':
            z = self._get_zipfile()
            try:
                #print(f"Opening file: {fname}")  # Debug print
                #print(f"File info: {z.getinfo(fname)}")  # Debug print
                return z.open(fname, 'r')
            except KeyError:
                print(f"File not found in zip archive: {fname}")
                raise
        return None
    ...
    def _load_raw_image(self, raw_idx: int) -> np.ndarray:
        ...
        try:
            with self._open_file(fname) as f:
                #print(f"Reading file: {fname}")  # Debug print
                # if reading was sucessfull, print sucess
                #print(f"Sucessfully read file: {fname}")

                image = PIL.Image.open(f)
                #print(f"Image format: {image.format}, mode: {image.mode}")
                
                image = np.array(image)
                #print(f"Loaded image shape: {image.shape}")  # Debug print

        except Exception as e:
            print(f"Error reading file {fname}: {e}")
            print(f'file info {f}')  # Debug print
            raise
        ...
        #print(f"Image shape before transpose: {image.shape}")  # Debug print
