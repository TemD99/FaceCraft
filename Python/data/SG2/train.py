...
import dnnlib
from Python.data.SG2.training import training_loop
...
def init_dataset_kwargs(data: str, resolution: int) -> dnnlib.EasyDict:
    ...
    if is_wds:
        ...
        d_kwargs.class_name = 'zip_utils.ImageFolderDataset'
...
@click.option('--batch-gpu',       help='Limit batch size per GPU',         type=click.IntRange(min=1), default=1)
# G architecture args
...
@click.option('--snap',            help='How often to save snapshots',      type=click.IntRange(min=1), default=5)
...
def main(**kwargs) -> None:
    ...
    # Networks.
    c.D_kwargs = dnnlib.EasyDict(class_name='models.discriminator.Discriminator')
    c.G_kwargs = dnnlib.EasyDict(class_name='models.generator.Generator', z_dim=64)
    ...
    # Loss.
    Loss = 'Python.data.SG2.training.losses.ProjectedGANLoss'
